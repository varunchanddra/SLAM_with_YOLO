...
bounding_box* yolodetect(network* net, metadata meta, char * img, int* nb_ptr){
      float thresh=.5;
        float hier_thresh=.5;
        float nms=.45;
        ...
        detection* dets = get_network_boxes(net, im.w, im.h, thresh, hier_thresh, NULL, 0, pnum);

        int n = *pnum;
    if (nms){
            do_nms_obj(dets, n, meta.classes, nms);
        }


        int max_boxes = 50;
        bounding_box* boxes = (bounding_box *)malloc(sizeof(bounding_box)*max_boxes);
   ...
   resizeStream(0)
{
    ...
    logFile = "/home/varun/Downloads/dyson_lab.klg";
	...
	// int num_boxes = 0;
 //    int* nb_ptr = &num_boxes;

    net = load_network("/home/varun/Humanoid/Project/semanticfusion/elasticfusionpublic/GUI/src/yolo/darknet/cfg/yolov3-tiny.cfg", 
        "/home/varun/Humanoid/Project/semanticfusion/elasticfusionpublic/GUI/src/yolo/darknet/weights/yolov3-tiny.weights", 0);
    meta = get_metadata("/home/varun/Humanoid/Project/semanticfusion/elasticfusionpublic/GUI/src/yolo/darknet/cfg/coco.data");
...
void MainController::launch()
{
    ...
    depthImage = cv::Mat(480, 640, CV_16UC1);
...
void MainController::addFrames(char * rgb, char * dep)
{
    mut.lock();

    cv::Mat one = cv::Mat(480, 640, CV_8UC3, rgb);
    one.copyTo(rgbImage);

    cv::Mat two = cv::Mat(480, 640, CV_16UC1, dep);
    two.copyTo(depthImage);

    mut.unlock();
    printf("adding frames");
    cv::imshow("COLOR", cv::Mat(480, 640, CV_8UC3, rgb));
    cv::imshow("DEP", cv::Mat(480, 640, CV_16UC1, dep));
    cv::waitKey(0);
}
    ...
    while(!pangolin::ShouldQuit() && !((!logReader->hasMore()) && quiet) && !(eFusion->getTick() == end && quiet))
    {
        if(!gui->pause->Get() || pangolin::Pushed(*gui->step))
        {
            if((logReader->hasMore() || rewind) && eFusion->getTick() < end)
            {
...
#if 1 // CHECKLIVE
                mut.lock();
                cv::Mat im = cv::Mat(480, 640, CV_8UC3);
                rgbImage.copyTo(im);

                cv::Mat dep = cv::Mat(480, 640, CV_16UC1);
                depthImage.copyTo(dep);
                mut.unlock();
                ...
                eFusion->processFrame((unsigned char *)im.data, (unsigned short int *)dep.data, ms, currentPose, weightMultiplier);
#else

                // printf("\ntime stamp is %ld\n", logReader->timestamp);
                printf("\n%d Depth values are %d %d\n", index, *( ( ( uint16_t *)logReader->depth )+340*240 ), *( ( ( uint16_t *)logReader->depth )+340*241 ) );
                cv::Mat im = cv::Mat(480, 640, CV_8UC3, logReader->rgb);

                eFusion->processFrame(logReader->rgb, logReader->depth, logReader->timestamp, currentPose, weightMultiplier);
#endif
                Eigen::Matrix4f pos = eFusion->getCurrPose();
...

                {
                    ...
                    // cv::Mat im = cv::Mat(480, 640, CV_8UC3, logReader->rgb);
                    cv::Mat ROI = im(cv::Rect(160, 120, 320, 240));
                    // cv::imshow("Color", ROI);

                    char name [156];
                    sprintf(name, "./imgs/im%d.jpeg",index);


                    cv::imwrite(name, ROI);  


    				bounding_box* detected_objects = yolodetect(net, meta, name, nb_ptr);
                    ...
                    {
                        bounding_box obj = detected_objects[k];
                        printf("BOUNDING BOX: %s; %f %f %f %f\n", obj.name, obj.bx.x, obj.bx.y, obj.bx.w, obj.bx.h);

                        int x = obj.bx.x ;
                        int y = obj.bx.y ;

                        float depth = *(logReader->depth + y * 640 + x);
                        Eigen::Vector4f point = pos * Eigen::Vector4f( 0, 0, depth, 1.0f);
                        int left = obj.bx.x+160 - obj.bx.w/2;
                        int top = obj.bx.y+120 - obj.bx.h/2;

                        cv::rectangle(im, cv::Rect( left, top, obj.bx.w, obj.bx.h), 
                                        cv::Scalar(0,0,255), 10);

                        std::cout << "The position of the object" << point;

                    }

                    free(detected_objects);
                }
                cv::imshow("Detector", im);
                cv::waitKey(10);

                // std::cout << "Current pose is " << pos << std::endl;
